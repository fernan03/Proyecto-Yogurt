...
public class YogurtMakingService {
    ...
    public YogurtBatch startNewBatch(Long recipeId, Double customMilkVolume, Double customStarterAmount){
        ...
        .orElseThrow(()-> new BusinessException("Recipe not found by this id :"+ recipeId));
        ...
        YogurtBatch saveBatch = batchRepository.save(batch);
        log.info("New yogurt batch started: {} with recipe: {}", saveBatch.getBatchCode(), recipe.getName());
    ...
    public YogurtBatch completeBatch(Long batchId){
        ...
        LocalDateTime expectedEnTime = batch.getRefrigerationStartTime()
            .plusHours(batch.getRecipe().getRefrigerationTime());
        
        if(LocalDateTime.now().isBefore(expectedEnTime)){
    ...
    public YogurtBatch markAsFailed(Long batchId, String reason){
        ...
        log.warn("Batch marked as failed: {}", batch.getBatchCode(), reason);
    ...
    public YogurtBatch getBatch(Long batchId){
        return batchRepository.findById(batchId)
            .orElseThrow(() -> new BusinessException("Batch not found by this id: " + batchId));
    }
