    ...
    private void maintainHeatingTemperature(YogurtBatch batch){
        new Thread(() -> {
            try {
                ...
                starCoolingProcess(batch);

            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                log.error("Heating mainteance interrupted for batch: {}", batch.getBatchCode());
            }
        }).start();
    }

    private void starCoolingProcess(YogurtBatch batch){
        log.info("Starting cooling for batch: {} to inoculation temperature: {}°C",
    ...
    public void startIncubationControl(YogurtBatch batch){
        ...
        new Thread(() -> {
            try {
                ...
                log.info("Incubation completed for batch: {}: {}°C", batch.getBatchCode());
